...
info "Waiting for OpenZiti Controller to become ready..."
wait_ready "helmrelease/ziti-controller" "flux-system" "5m"

info "Waiting for OpenZiti Router Enrollment Job to complete..."
if ! kubectl wait --for=condition=complete job/ziti-router-enroll -n openziti --timeout=5m; then
  echo "ERROR: Timed out waiting for ziti-router-enroll job in openziti"
  kubectl describe job ziti-router-enroll -n openziti || true
  kubectl logs -n openziti -l job-name=ziti-router-enroll || true
  exit 1
fi
success "OpenZiti Router Enrollment Job completed."

info "Checking if enrollment secret was created..."
require_resource "secret/ziti-router-enrollment" "openziti"

info "Waiting for OpenZiti Router to become ready..."
wait_ready "helmrelease/ziti-router" "flux-system" "5m"

echo -e "\n${GREEN}${BOLD}  ✔  OpenZiti validation test completed successfully!${RESET}\n"
