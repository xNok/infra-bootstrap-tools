...
if [ -n "$$AUTH0_ISSUER" ] && [ -n "$$AUTH0_JWKS_ENDPOINT" ] && [ -n "$$AUTH0_AUDIENCE" ]; then
    if ! ziti edge list ext-jwt-signers 'name="auth0"' | grep -q "auth0"; then
        ...
        ziti edge create ext-jwt-signer "auth0" "$$AUTH0_ISSUER" -u "$$AUTH0_JWKS_ENDPOINT" --audience "$$AUTH0_AUDIENCE" --claims-property "email"
    else
    ...
    if ! ziti edge list auth-policies 'name="auth0-auth"' | grep -q "auth0-auth"; then
        ...
        AUTH0_SIGNER_ID=$$(ziti edge list ext-jwt-signers 'name="auth0"' -j | grep -o '"id":"[^"]*' | awk -F '"' '{print $$4}')
        ziti edge create auth-policy "auth0-auth" --primary-ext-jwt-allowed --primary-ext-jwt-allowed-signers "$$AUTH0_SIGNER_ID"
    else
    ...
    if ! ziti edge list auth-policies 'name="keycloak-auth"' | grep -q "keycloak-auth"; then
        ...
        KEYCLOAK_SIGNER_ID=$$(ziti edge list ext-jwt-signers 'name="keycloak"' -j | grep -o '"id":"[^"]*' | awk -F '"' '{print $$4}')
        ziti edge create auth-policy "keycloak-auth" --primary-ext-jwt-allowed --primary-ext-jwt-allowed-signers "$$KEYCLOAK_SIGNER_ID"
    else
