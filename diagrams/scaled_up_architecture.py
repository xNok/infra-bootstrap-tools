...
with Diagram(name="Scaled up infra for small self hosted project", show=False):
    ...
    with Cluster("Applications worker", direction="TB"):

        portainer_agent = Custom("portainer Agent", "/out/portainer.png")
        portainer >> portainer_agent

        with Cluster("Application 1", direction="TB"):
        ...
        with Cluster("Application 2", direction="TB"):
            app = Server("Application")
            db  = Mongodb("Database")
