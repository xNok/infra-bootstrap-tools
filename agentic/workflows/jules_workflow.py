...
async def initialize_mcp_toolset(mcp_server_url: str) -> FastMCPToolset:
    ...
    if not is_safe_mcp_url(mcp_server_url):
        raise ValueError(f"Insecure or invalid MCP server URL provided: {mcp_server_url}")

    try:
        # ⚡ Bolt Optimization: FastMCPToolset performs synchronous network I/O
        # Wrapping in asyncio.to_thread prevents blocking the Prefect async event loop,
        # allowing other concurrent tasks/flows to execute without stalling.
        toolset = await asyncio.to_thread(FastMCPToolset, mcp_server_url)
        return toolset
...
def create_jules_agent(
    toolset: FastMCPToolset, model: str = "google-gla:gemini-1.5-flash"
) -> Agent:
    """
...
async def jules_agent_workflow(
    ...
    toolset = await initialize_mcp_toolset(mcp_url)
